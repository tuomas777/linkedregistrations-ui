...
const testAboveMinAge = async (minAge: string, date: string) => {
  try {
    await isAboveMinAge(minAge, Yup.string()).validate(date);
    return true;
  } catch (e) {
    return false;
  }
...
const testBelowMaxAge = async (maxAge: string, date: string) => {
  try {
    await isBelowMaxAge(maxAge, Yup.string()).validate(date);
    return true;
  } catch (e) {
    return false;
  }
...
describe('attendeeSchema function', () => {
  ...
  test('should return false if date of birth is missing', async () => {
    expect(
      await testAttendeeSchema({ ...validAttendee, dateOfBirth: '' })
    ).toBe(false);
  });
